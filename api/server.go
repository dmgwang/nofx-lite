...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		protected := api.Group("/", s.authMiddleware())
		{
			...
			protected.GET("/models/:id", s.handleGetModelConfig)
			protected.PUT("/models", s.handleUpdateModelConfigs)

			// 交易所配置
			protected.GET("/exchanges", s.handleGetExchangeConfigs)
			protected.GET("/exchanges/:id", s.handleGetExchangeConfig)
...
// handleGetModelConfig 获取单个AI模型配置（包含敏感信息）
func (s *Server) handleGetModelConfig(c *gin.Context) {
	userID := c.GetString("user_id")
	modelID := c.Param("id")
	...
	log.Printf("🔍 查询用户 %s 的AI模型 %s 配置", userID, modelID)
	...
	// 查找指定的模型
	var targetModel *config.AIModelConfig
	...
	if targetModel == nil {
		log.Printf("❌ 未找到AI模型 %s 的配置", modelID)
		c.JSON(http.StatusNotFound, gin.H{"error": "AI模型配置不存在"})
		return
	}

	log.Printf("✅ 找到AI模型 %s 的配置", modelID)
	...
	result := map[string]interface{}{
		...
		"customApiUrl":    targetModel.CustomAPIURL,
		"customModelName": targetModel.CustomModelName,
...
// handleGetExchangeConfig 获取单个交易所配置（包含敏感信息）
func (s *Server) handleGetExchangeConfig(c *gin.Context) {
	userID := c.GetString("user_id")
	...
	log.Printf("🔍 查询用户 %s 的交易所 %s 配置", userID, exchangeID)
	...
	// 查找指定的交易所
	var targetExchange *config.ExchangeConfig
	...
	if targetExchange == nil {
		log.Printf("❌ 未找到交易所 %s 的配置", exchangeID)
		c.JSON(http.StatusNotFound, gin.H{"error": "交易所配置不存在"})
		return
	}

	log.Printf("✅ 找到交易所 %s 的配置", exchangeID)

	// 返回完整的配置信息（包含敏感数据）
	result := map[string]interface{}{
		"id":                    targetExchange.ID,
		"name":                  targetExchange.Name,
		"type":                  targetExchange.Type,
		"enabled":               targetExchange.Enabled,
		"apiKey":                targetExchange.APIKey,
		"secretKey":             targetExchange.SecretKey,
		"testnet":               targetExchange.Testnet,
		"hyperliquidWalletAddr": targetExchange.HyperliquidWalletAddr,
		"asterUser":             targetExchange.AsterUser,
		"asterSigner":           targetExchange.AsterSigner,
		"asterPrivateKey":       targetExchange.AsterPrivateKey,
		"createdAt":             targetExchange.CreatedAt,
		"updatedAt":             targetExchange.UpdatedAt,
	}
...
func (s *Server) handleAccount(c *gin.Context) {
	...
	log.Printf("📊 收到账户信息请求 [%s] (traderID: %s)", trader.GetName(), traderID)
	...
	if err != nil {
		...
		if strings.Contains(errorMsg, "connection") || strings.Contains(errorMsg, "network") {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "网络连接失败，请检查交易所连接配置",
			})
		} else if strings.Contains(errorMsg, "authentication") || strings.Contains(errorMsg, "API key") {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "API密钥验证失败，请检查交易所API配置",
			})
		} else if strings.Contains(errorMsg, "testnet") {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "测试网连接失败，请检查测试网配置",
			})
		} else {
			c.JSON(http.StatusInternalServerError, gin.H{
