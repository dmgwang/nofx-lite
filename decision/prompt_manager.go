...
func GetPromptTemplate(name string) (*PromptTemplate, error) {
	...
	// 记录所有可用模板名称用于调试
	allNames := GetAllPromptTemplateNames()
	log.Printf("📚 可用模板列表: %v", allNames)
	
	template, err := globalPromptManager.GetTemplate(name)
	if err != nil {
		return nil, fmt.Errorf("模板 '%s' 未找到。可用模板: %v", name, allNames)
	}
