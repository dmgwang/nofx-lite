...
func (t *FuturesTrader) GetBalance() (map[string]interface{}, error) {
	...
	log.Printf("ℹ️  当前测试网模式: %v", futures.UseTestnet)
	account, err := t.client.NewGetAccountService().Do(context.Background())
	if err != nil {
		log.Printf("❌ 币安API调用失败: %v", err)
		// 提供更详细的错误信息
		if strings.Contains(err.Error(), "connection") || strings.Contains(err.Error(), "timeout") {
			return nil, fmt.Errorf("网络连接失败，请检查网络连接或测试网配置: %w", err)
		} else if strings.Contains(err.Error(), "API key") || strings.Contains(err.Error(), "signature") {
			return nil, fmt.Errorf("API密钥验证失败，请检查API密钥配置: %w", err)
		} else if strings.Contains(err.Error(), "testnet") {
			return nil, fmt.Errorf("测试网连接失败，请确认测试网API密钥是否正确: %w", err)
		}
