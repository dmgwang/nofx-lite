...
This is an open-source AI trading system for learning and research purposes.
...
NOFX-Lite now supports **three major exchanges**: Binance, Hyperliquid, and Aster DEX!
...
NOFX-Lite is currently **fully operational in cryptocurrency markets** with the following proven capabilities:
...
- **Flexible Prompt Templates**: Support for multiple system prompt templates (default, aggressive, custom)
- **Real-time Template Display**: Current prompt template name shown in trader status bar
- **Template Management**: Easy switching between different trading strategies via web interface
- **Custom Templates**: Create and manage custom prompt templates for specialized trading approaches
- **Template-based Decision Making**: AI uses selected template for generating trading decisions
...
NOFX-Lite is on a mission to become the **Universal AI Trading Operating System** for all financial markets.
...
**NOFX-Lite is the lightweight version of NOFX**, designed for simplicity and ease of use while maintaining the core AI trading capabilities.

**Upcoming Features:**

- Enhanced AI capabilities (GPT-4, Claude 3, Gemini Pro, flexible prompt templates) ✅ **Implemented**
...
📖 **For detailed roadmap and timeline, see the project roadmap.**

---

## 🏗️ Technical Architecture

NOFX-Lite is built with a modern, modular architecture:
...
📖 **For detailed architecture documentation, see the project architecture section.**

---

## 🔐 Admin Mode (Single-User)

For self-hosted or single-tenant setups, NOFX-Lite supports a strict admin-only mode that disables public features and requires an admin password for all access.
...
5. **Create API Key**:
   ...
   - Save API Key and Secret Key (~~needed for config.json~~) *needed for NOFX-Lite web interface*
...
**📖 For detailed Docker deployment guide, troubleshooting, and advanced configuration, see the deployment section below.**
...
```bash
git clone https://github.com/dmgwang/nofx-lite.git
cd nofx-lite
```
...
- **Prompt Template Display**: Shows current system prompt template name in trader status bar
...
GET  /api/prompt-templates/default   # Get default prompt template
```
...
> 📖 **For detailed troubleshooting:** See the troubleshooting section below.
...
📖 **For detailed version history and updates, see the changelog.**
...
- **GitHub Issues**: [Submit an Issue](https://github.com/tinkle-community/nofx-lite/issues)
- **Developer Community**: [Telegram Group](https://t.me/nofxlite_dev_community)
