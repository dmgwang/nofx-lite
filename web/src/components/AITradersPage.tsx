...
function ModelConfigModal({
  ...
  // 如果是编辑现有模型，从API获取完整的模型配置（包含敏感信息）
  useEffect(() => {
    const loadModelConfig = async () => {
      if (editingModelId) {
        setLoading(true)
        try {
          const fullModelConfig = await api.getModelConfig(editingModelId)
          setApiKey(fullModelConfig.apiKey || '')
          setBaseUrl(fullModelConfig.customApiUrl || '')
          setModelName(fullModelConfig.customModelName || '')
        } catch (error) {
          console.error('Failed to load model config:', error)
          // 如果API调用失败，使用已有的非敏感数据作为备选
          const existingModel = configuredModels?.find((m) => m.id === editingModelId)
          if (existingModel) {
            setApiKey('') // API密钥不会从非敏感数据中获取
            setBaseUrl(existingModel.customApiUrl || '')
            setModelName(existingModel.customModelName || '')
          }
        } finally {
          setLoading(false)
        }
      ...
      >
        ...
        {loading && (
          <div className="mb-4 p-3 rounded" style={{ background: '#0B0E11', border: '1px solid #2B3139' }}>
            <div className="flex items-center gap-2">
              <div className="animate-spin rounded-full h-4 w-4 border-b-2" style={{ borderColor: '#F0B90B' }}></div>
              <span className="text-sm" style={{ color: '#EAECEF' }}>正在加载模型配置...</span>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button
              type="submit"
              disabled={!selectedModel || !apiKey.trim() || loading}
              className="flex-1 px-4 py-2 rounded text-sm font-semibold disabled:opacity-50"
              style={{ background: '#F0B90B', color: '#000' }}
            >
              {loading ? '加载中...' : t('saveConfig', language)}
            </button>
          </div>
        </form>
      </div>
...
function ExchangeConfigModal({
  ...
  // 如果是编辑现有交易所，从API获取最新配置数据
  useEffect(() => {
    if (editingExchangeId) {
      const fetchExchangeConfig = async () => {
        try {
          const exchangeConfig = await api.getExchangeConfig(editingExchangeId)
          setApiKey(exchangeConfig.apiKey || '')
          setSecretKey(exchangeConfig.secretKey || '')
          setPassphrase(exchangeConfig.passphrase || '') // Load passphrase from API
          setTestnet(exchangeConfig.testnet || false)

          // Aster 字段
          setAsterUser(exchangeConfig.asterUser || '')
          setAsterSigner(exchangeConfig.asterSigner || '')
          setAsterPrivateKey(exchangeConfig.asterPrivateKey || '') // Load private key from API

          // Hyperliquid 字段
          setHyperliquidWalletAddr(exchangeConfig.hyperliquidWalletAddr || '')
        } catch (error) {
