...
export function TraderConfigModal({
  ...
  useEffect(() => {
    if (traderData) {
      // 编辑模式：使用传入的数据，优先保留当前配置值
      const currentSystemPromptTemplate = traderData.system_prompt_template
      
      // 日志输出当前配置值
      console.log('初始化交易员配置 - 当前system_prompt_template值:', currentSystemPromptTemplate)
      
      // 验证当前值是否有效，仅当无效时才使用默认值
      const isValidTemplate = currentSystemPromptTemplate && 
                             currentSystemPromptTemplate !== null && 
                             currentSystemPromptTemplate !== undefined && 
                             currentSystemPromptTemplate !== ''
      
      const finalTemplate = isValidTemplate ? currentSystemPromptTemplate : 'default'
      
      console.log('初始化交易员配置 - 最终使用的system_prompt_template值:', finalTemplate)
      
      setFormData({
        ...traderData,
        system_prompt_template: finalTemplate,
      })
  ...
  const handleFetchCurrentBalance = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        // 尝试获取更详细的错误信息
        let errorMessage = '获取账户余额失败'
        try {
          const errorData = await response.json()
          errorMessage = errorData.error || `HTTP ${response.status}: ${response.statusText}`
        } catch (e) {
          // 如果无法解析JSON错误信息，使用状态文本
      ...
      setBalanceFetchError(`获取余额失败: ${errorMessage}`)
    } finally {
  ...
  const handleSave = async () => {
    ...
    // 验证system_prompt_template值
    const currentTemplateValue = formData.system_prompt_template
    console.log('保存交易员配置 - 当前system_prompt_template值:', currentTemplateValue)
    
    // 检查值是否有效，如果无效则使用默认值
    const isValidTemplate = currentTemplateValue && 
                           currentTemplateValue !== null && 
                           currentTemplateValue !== undefined && 
                           currentTemplateValue !== ''
    
    if (!isValidTemplate) {
      console.warn('保存交易员配置 - system_prompt_template值无效，将使用默认值: default')
    ...
    try {
      ...
      console.log('保存交易员配置 - 最终保存的system_prompt_template值:', saveData.system_prompt_template)
