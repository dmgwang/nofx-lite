...
export const api = {
  ...
  // 获取单个AI模型配置（包含敏感信息）
  async getModelConfig(modelId: string): Promise<AIModel> {
    const res = await httpClient.get(`${API_BASE}/models/${modelId}`, getAuthHeaders())
    if (!res.ok) throw new Error('获取模型配置失败')
    return res.json()
  },
  ...
  // 获取单个交易所配置（包含敏感信息）
  async getExchangeConfig(exchangeId: string): Promise<Exchange> {
    const res = await httpClient.get(`${API_BASE}/exchanges/${exchangeId}`, getAuthHeaders())
    if (!res.ok) throw new Error('获取交易所配置失败')
    return res.json()
  },
