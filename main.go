...
func main() {
	fmt.Println("╔════════════════════════════════════════════════════════════╗")
	fmt.Println("║    🤖 NOFX-Lite AI Trading System - DeepSeek & Qwen      ║")
	fmt.Println("╚════════════════════════════════════════════════════════════╝")
	fmt.Println()
	...
	// 1. 优先从环境变量 NOFX_LITE_BACKEND_PORT 读取
	if envPort := strings.TrimSpace(os.Getenv("NOFX_LITE_BACKEND_PORT")); envPort != "" {
		if port, err := strconv.Atoi(envPort); err == nil && port > 0 {
			apiPort = port
			log.Printf("🔌 使用环境变量端口: %d (NOFX_LITE_BACKEND_PORT)", apiPort)
		} else {
			log.Printf("⚠️  环境变量 NOFX_LITE_BACKEND_PORT 无效: %s", envPort)
		}
	} else if apiPortStr != "" {
